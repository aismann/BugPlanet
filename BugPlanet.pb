...
; -----------------------------------------
; BugsPlanet v1.1 icesoft 20250831
;  Controls: W, A S, D, E, (Arrows), H,  Leftmouse, Rightmouse.
; * Bug fixes 
; * Source code redesign (clean code) 
...
Procedure app_start()
  ...
  ; generate the content of the egg
  For x = #RESS To #RESE
    test = Random(4,1)
    If test = 4
    ...
    If MouseButton(#PB_MouseButton_Left) Or KeyboardPushed(#PB_Key_E)
      ...
      If rayhitbool
        If IsEntity(rayhitbool)
          If p\box<5 And object(rayhitbool)\id=#BOX
...
;     EndIf
          ...
          If IsEntity(rayhitbool)
            ...
            If Abs(distent)<Abs(dist)+distb
              ...
              If rayhitbool<#BGE
                ...
                object(rayhitbool)\armor-(Random(p\dmgmax,p\dmgmin))
                ...
                If object(rayhitbool)\armor<1
                  ...
                  ElseIf object(rayhitbool)\id = #Egg
                    ...
                    EntityAngularFactor(rayhitbool,0,1,0) ;x  //fixed
                    EntityLinearFactor(rayhitbool,1,0,1)  ;x  //fixed
                    PlaySound(crunch)
  ...
  If p\won = 1
    ...
    petskii::ctobject(ScreenWidth()/2,150,"You fired "+Str(p\fired)+" shots.",c3,c2)
    petskii::ctobject(ScreenWidth()/2,180,"You killed "+Str(p\kills)+" bugs.",c3,c2)
    petskii::ctobject(ScreenWidth()/2,210,"You accidentally destroyed "+Str(p\boxdestroyed)+" crates.",c3,c2)
    petskii::ctobject(ScreenWidth()/2,240,"You collected "+Str(p\collected)+" crates,",c3,c2)
    petskii::ctobject(ScreenWidth()/2,270,"You uploaded "+Str(p\load)+" crates.",c3,c2)
    petskii::ctobject(ScreenWidth()/2,300,"You spent "+Str(p\spentarmor)+" crates on armor and "+Str(p\spentammo)+" on ammo.",c3,c2)
    If object(12)\armor>0
    ...
    If mins = 0 And seconds = 0
      ...
      score = 5000+(p\kills*8) + p\fired -(p\boxdestroyed*10) + p\collected*10 + (p\load*50)-p\spentarmor-p\spentammo-object(12)\armor
    EndIf
  ...
  If p\armor<=0 And p\won=0
    ...
    If mins = 0 And seconds = 0
      mins = (ElapsedMilliseconds()/1000)/60
      seconds = Mod(ElapsedMilliseconds()/1000,60)
      score = (p\kills*8) + p\fired -(p\boxdestroyed*10) + p\collected*10 + (p\load*50)-p\spentarmor-p\spentammo-object(12)\armor
    EndIf
    petskii::ctobject(ScreenWidth()/2,380,"You survived "+Str(mins)+" minutes and "+Str(seconds)+" seconds.",c3,c2)
    petskii::ctobject(ScreenWidth()/2,410,"Your Score is "+Str(score)+".",c3,c2)
  Else
    petskii::textoutlined(0,0,"NEST HEALTH : "+Str(object(12)\armor)+"/1500",c1,c2)
